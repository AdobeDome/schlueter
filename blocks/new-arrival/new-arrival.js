...
function buildCard(item, isAuthor, recommendedPath) {
  ...
  // Make card clickable and redirect to the product detail page
  if (slug) {
  ...
  if (highlight) {
    ...
    highlightEl.className = "na-card-highlight";
...
async function fetchProducts(folderPath) {
  try {
    if (!folderPath) return [];
    // Cache-bust via query param: the AEM publish CDN caches this response without
    // varying by Origin, so a stale cached hit can be missing the CORS header, and
    // Cache-Control/Pragma request headers would trigger a preflight this endpoint rejects.
    ...
    return items.filter((item) => item && item._path);
  } catch (e) {
...
function filterProductsBySKU(products, skuList) {
  ...
  return products.filter((product) => normalizedSKUs.includes(getProductSlug(product._path).toLowerCase()));
}
...
export default async function decorate(block) {
  ...
  const recommendedPath = cfg?.['na-recommended-path'] || '/product-overview';

  // Extract SKUs (content fragment names) from multifield
  const skuList = extractSKUs(block, cfg);

  // Clear author table
  block.innerHTML = "";

  // Fetch all products from the folder
  ...
  // Filter products by SKU (content fragment name)
  ...
  const cards = filteredProducts.map((item) => buildCard(item, isAuthor, recommendedPath));
